...
export default function CheckoutModal({ isOpen, onClose }: CheckoutModalProps) {
    ...
    try {
      ...
      if (useAppsScript) {
        if (appsScriptUrl && appsScriptUrl.trim()) {
          ...
          console.warn("VITE_APPS_SCRIPT_URL is not configured.");
          setSheetError("Lỗi: Chưa cấu hình đường dẫn Google Sheets Web App. Vui lòng thiết lập biến môi trường VITE_APPS_SCRIPT_URL hoặc thêm ?admin=true vào URL trang web để mở bảng cấu hình dán link!");
            ...
            <div className="overflow-y-auto p-6 sm:p-8 flex-1">
              {!isSuccess ? (
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  {/* Administrative System Configuration Button - Only visible in ?admin=true mode */}
                  {isAdminMode && (
                    <div className="pt-2 border-t border-gray-100 mt-2">
                      <button
                        ...
                        className="w-full text-gray-400 hover:text-gray-600 transition-colors py-2 flex items-center justify-center gap-1.5 text-[11px] font-mono font-semibold tracking-wider border border-gray-200 hover:bg-gray-50 rounded-xl uppercase"
                      >
  ...
  try {
    ...
    if (!sheet) {
      ...
      sheet.appendRow(["HỌ VÀ TÊN", "SỐ ĐIỆN THOẠI", "ĐỊA CHỈ GMAIL", "ĐỊA CHỈ NHẬG HÀNG", "GHI CHÚ", "SỐ LƯỢNG", "THỜI GIAN"]);
    }
    ...
    return ContentService.createTextOutput(JSON.stringify({ "status": "error", "message": error.toString() }))
      .setMimeType(ContentService.MimeType.JSON);
                            ...
                            <button
                              ...
                              className="w-full bg-zinc-900 hover:bg-zinc-800 text-white text-xs font-bold py-2.5 rounded-xl transition-colors flex items-center justify-center gap-1.5 cursor-pointer text-center"
                            >
                              {adminSaveSuccess ? (
                                <>
